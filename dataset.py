...
class SunDataset_Supervised(data.Dataset):
    ...
    def __getitem__(self, index):
        if self.split == 'train':
            ...
            anchor, positive = get_positive(self.root, anc_id, pos_id, self.num_patches)
...
def get_positive(root, anc_id, pos_id, num_patches):
    ...
    anchor_neighbor = collect_local_neighbor(anc_pts, anc_pcd)
    ...
    positive_neighbor = collect_local_neighbor(pos_pts, pos_pcd)
